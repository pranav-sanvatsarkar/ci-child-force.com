...
sforce.Transport = function(url) {
    ...
    this.send = function (envelope, callback, async, timeout, progressTracker) {
        this.newConnection();
        if (async) {
        ...
        //this.connection.setRequestHeader("Content-Type", "multipart/form-data");
        this.connection.setRequestHeader("SOAPAction", "\"\"");
        this.connection.setRequestHeader("Accept", "text/xml");
        this.connection.setRequestHeader("User-Agent", "SFAJAX 1.0");
        this.connection.upload.addEventListener("progress", progressTracker, false);
...
sforce.SoapTransport.prototype.send = function(url, writer, isArray, connectionCallback, progressTracker) {
    this.isArray = isArray;
    var transport = new sforce.Transport(url);
    this.isAsync = connectionCallback ? true : false;
    if (this.isAsync) {
        this.connectionCallback = connectionCallback;
        transport.send(writer.toString(), this, this.isAsync, connectionCallback.timeout, progressTracker);
    } else {
...
sforce.Connection.prototype.create = function (sobjects, callback, progressTracker) {
    var arg = new sforce.internal.Parameter("sObjects", sobjects, true);
    return this.invoke("create", [arg], true, callback, progressTracker);
};
...
sforce.Connection.prototype.invoke = function(method, args, isArray, callback, progressTracker) {
    return this._invoke(method, args, isArray, callback, this.namespaceMap, this.serverUrl, this.sforceNs, this.sobjectNs, progressTracker);
};

sforce.Connection.prototype._invoke = function(method, args, isArray, callback, namespaces, url, headerNs, sobjectNs, progressTracker) {
    if (callback) {
    ...
    return transport.send(url, writer, isArray, callback, progressTracker);
};
